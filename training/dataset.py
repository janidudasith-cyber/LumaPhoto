...
# ---------------------------------------------------------------------------
# LOL — Low-Light dataset (485 training pairs)
# ---------------------------------------------------------------------------

class LOLDataset(Dataset):
    """
    LOL (Low-Light) dataset — 485 training + 15 evaluation pairs.
    Real low-light photographs paired with normal-light references.
    ...
    Kaggle dataset:  soumikrakshit/lol-dataset  (add to notebook, ~1 GB)
    """

    def __init__(self, root: str, crop_size: int = 256, train: bool = True):
        split = "our485" if train else "eval15"
        self.low_dir  = Path(root) / split / "low"
        self.high_dir = Path(root) / split / "high"

        if not self.low_dir.exists() or not self.high_dir.exists():
            raise FileNotFoundError(
                f"LOL dirs not found under '{root}/{split}/'\n"
                "Add the Kaggle dataset 'soumikrakshit/lol-dataset' to your notebook."
            )

        self.files = sorted(
            f.name for f in self.low_dir.iterdir()
    ...
    def __getitem__(self, idx: int):
        name = self.files[idx]
        inp  = Image.open(self.low_dir  / name).convert("RGB")
        tgt  = Image.open(self.high_dir / name).convert("RGB")
...
def build_dataset(
    ...
    if lol_root and Path(lol_root).exists():
        try:
            ds = LOLDataset(lol_root, crop_size=crop_size, train=train)
            datasets.append(ds)
            print(f"LOL: {len(ds)} pairs")
        except FileNotFoundError as e:
            print(f"LOL skipped: {e}")
