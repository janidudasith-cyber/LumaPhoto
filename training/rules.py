...
def analyze(img: Image.Image) -> dict:
    ...
    # Skin tone — rg cap tightened to 1.45 so sandy/earthy warm tones don't match
    rg_ratio = r / np.maximum(g, 1.0)
    skin = ((r > 100) & (r > g) & (g > b) & (r > b * 1.15) &
            (rg_ratio >= 1.05) & (rg_ratio <= 1.45) & (lum > 60) & (lum < 210))
    ...
    if l_mean < 42 and dark_ratio > 0.55 and 0.004 < hotspot_ratio < 0.18:
        scene = "Night"
    elif l_std > 60 and dark_ratio > 0.10 and bright_ratio > 0.10:
        scene = "HDR"
    elif sky_ratio > 0.22 or green_ratio > 0.30:
        scene = "Landscape"   # strong landscape beats portrait
    elif skin_ratio > 0.12 and sky_ratio < 0.15 and green_ratio < 0.20:
        scene = "Portrait"
